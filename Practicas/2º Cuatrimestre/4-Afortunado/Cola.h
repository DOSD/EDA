...
public:
	...
	/*
		devuelve la cola con un solo elemente descartando m-ésimos
		Lo hace colocando los salvados al final de la cola y borrando los m-esimos
	*/
	void escogeAfortunadoCircular(unsigned int m){
		if (esVacia())
			throw EColaVacia();
		unsigned int i = 1;
		while(_prim != _ult){
	...
	/*
		devuelve la cola con un solo elemento descartando m-ésimos
		Lo hace mediante la busqueda de elementos m-esimos y borrandolos directamente. Se enlaza el anterior al elemento borrado con el posterior si los hay.
	*/
	void escogeAfortunado(unsigned int m) {
		if (esVacia())
			throw EColaVacia();
		...
		while(_prim != _ult){
			...
			else if(aBorrar == _prim){
				_prim = aBorrar->_sig;
				ultimo = _prim;
			}
			else{ 
				ultimo->_sig = aBorrar->_sig;
				ultimo = ultimo->_sig;
			}
			delete aBorrar;
		}
